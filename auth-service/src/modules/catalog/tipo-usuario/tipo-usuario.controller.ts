...
export class TiposUsuarioController {
  ...
  async crearTipoUsuario(@Body() tipoUsuarioDto: CrearTipoUsuarioDto) {
    try {
      const tipoUsuario = await this.tiposUsuarioService.create(tipoUsuarioDto);
      return {
        status: 'success',
  ...
  async obtenerTiposUsuario() {
    return await this.tiposUsuarioService.findAll();
  }
  ...
  async obtenerTipoUsuarioPorId(@Param('id', ParseIntPipe) id: number) {
    return await this.tiposUsuarioService.findOne(id);
  }
  ...
  async actualizarTipoUsuario(
    ...
    return await this.tiposUsuarioService.update(id, dto);
  }
  ...
  async eliminarTipoUsuario(@Param('id', ParseIntPipe) id: number) {
    return await this.tiposUsuarioService.remove(id);
  }
}
